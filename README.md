...
1. **Never store passwords in plain text** - Use proper hashing (bcrypt, argon2)
2. **Use a backend API** - Don't store sensitive data locally
3. **Implement JWT tokens** - Not just user objects
4. **Use HTTPS** - Always encrypt data in transit
5. **Validate on backend** - Don't trust client-side validation alone
6. **Rate limit** - Prevent brute force attacks
7. **Sanitize inputs** - Prevent injection attacks

### Design Decisions

- **Single state object**: Cleaner than multiple `useState` calls
- **Result objects**: More predictable than throwing errors
- **Path aliases**: Easier to maintain and refactor
- **Inline validation**: Better UX than submit-only validation
- **TypeScript**: Type safety catches errors early
- **Context API**: Sufficient for this app size (Redux would be overkill)
